...
{
    if($donnees['nom_categorie']=='EPILATION')
    {
         ?>
            ...
            <ul class="product-filter-menu">
                 ...
                 $bdd = new  PDO('mysql:host=localhost;dbname=id12825075_pcosm;charset=utf8', 'id12825075_admin', 'pcosm123456');
$i=10;
    $reponse = $bdd->query("SELECT * FROM sous_categorie WHERE id_categorie=3");
while($donnees= $reponse->fetch())   
{
    if($donnees['nom_scategorie']=='Crème & produits dépilatoire')
    {
         ?>
                				
                <li style="	margin: 0;"><a href="portfolio<?php echo $donnees['id_categorie']-1 ?>.php" style="color: purple; font-size: 11px;"><?php echo $donnees['nom_scategorie'] ?></a></li>
                <?php
    }
    else{
                ?>
                
				<li><a href="portfolio<?php echo $i ?>.php" ><?php echo $donnees['nom_scategorie'] ?></a></li>
                <?php
                    $i++;
    }
}
			...
			<div class="row">
                 ...
                 $bdd = new  PDO('mysql:host=localhost;dbname=id12825075_pcosm;charset=utf8', 'id12825075_admin', 'pcosm123456');
$i=0;
    $reponse = $bdd->query("SELECT * FROM produit WHERE id_scategorie=1");
while($donnees= $reponse->fetch())   
{
        $reponse2 = $bdd->query("SELECT * FROM sous_categorie WHERE id_scategorie=1");
        ...
        while($donnees3= $reponse3->fetch())   
{
   ...
   ?>
            ...
            $_SESSION['pf'] = "2"; ?>"><i class="fa fa-shopping-bag"></i><span>Add to Cart</span></a>
							</div>
						</div>
						<div class="pi-text">
