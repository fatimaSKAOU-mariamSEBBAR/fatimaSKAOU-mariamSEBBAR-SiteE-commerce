...
while($donnees= $reponse->fetch())   	 			
{
    if($donnees['nom_scategorie']=='Eau de toilette')
    {
	 ...
	 <?php
                 $bdd = new PDO('mysql:host=localhost;dbname=id12825075_pcosm;charset=utf8', 'id12825075_admin', 'pcosm123456');
$i=0;
    $reponse = $bdd->query("SELECT * FROM produit WHERE id_scategorie=16");
while($donnees= $reponse->fetch())    {  
        $reponse2 = $bdd->query("SELECT * FROM sous_categorie WHERE id_scategorie=16");
        ...
        while($donnees3= $reponse3->fetch())   
{
   ...
   ?>
            ...
            $_SESSION['pf'] = "5"; ?>"><i class="fa fa-shopping-bag"></i><span>Add to Cart</span></a>
							</div>
						</div>
						<div class="pi-text">
